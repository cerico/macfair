...
if [ ! -f "$COMPONENTS_TARBALL" ]; then
  echo "📦 shadcn-components.tar.gz not found. Downloading..."
  curl -L -o "$COMPONENTS_TARBALL" \
...
# tailwind v3 required for shadcn/ui compatibility and tailwind.config.js approach
pnpm install -D tailwindcss@3.4.1 postcss autoprefixer @types/node tailwindcss-animate json5
...
try {
  ...
  console.error('Failed to parse tsconfig.app.json:', err.message);
  process.exit(1);
}
"
...
echo "📦 Copying bundle script..."
mkdir -p scripts
cp "$SCRIPT_DIR/bundle-artifact.sh" scripts/

echo "✅ Setup complete!"
echo ""
echo "📦 Included: 40+ shadcn/ui components"
echo ""
echo "To start developing:"
echo "  cd $PROJECT_NAME"
echo "  pnpm dev"
echo ""
echo "To bundle for sharing:"
