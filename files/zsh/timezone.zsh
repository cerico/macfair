...
autotz() {
  ...
  local current_tz=$(sudo systemsetup -gettimezone 2>/dev/null | awk '{print $3}')
  ...
  if [[ "$current_tz" != "$target_tz" ]]; then
    ...
    sudo systemsetup -settimezone "$target_tz" 2>&1 | grep -v "### Error:-99"
  fi
}
...
settz() {
  case "$1" in
    "sarajevo"|"weekend"|"bih") 
      sudo systemsetup -settimezone "Europe/Sarajevo" 2>&1 | grep -v "### Error:-99"
      echo "Timezone set to Europe/Sarajevo"
      ;;
    "london"|"weekday"|"uk")
      sudo systemsetup -settimezone "Europe/London" 2>&1 | grep -v "### Error:-99"
      echo "Timezone set to Europe/London"
      ;;
    *)
      echo "Usage: settz [sarajevo|london]"
      echo "Current timezone: $(sudo systemsetup -gettimezone 2>/dev/null | awk '{print $3}')"
      ;;
  esac
}
