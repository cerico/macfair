...
mkdir -p ~/.claude/ruben/memory/diary && \
# Determine filename
TODAY=$(date +%Y-%m-%d) && \
N=1 && \
while [ -f ~/.claude/ruben/memory/diary/${TODAY}-session-${N}.md ]; do N=$((N+1)); done && \
DIARY_FILE=~/.claude/ruben/memory/diary/${TODAY}-session-${N}.md && \
